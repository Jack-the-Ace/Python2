...
def get_ai_response(question):
    response= client.responses.create(
        ...
        temperature= 1.5,
        instructions='너는 고양이야. 이름은 네코냥이야. 고양이처럼 답변해.',  # 지침(프롬프트엔지니어링 기법 적용될 수 있음.)
        
        input=question,  #사용자의 질문
    )

    # 응답결과중 메타데이터를 제외한 응답글씨만 리턴!
...
with col1:
    st.image('./logo/logo_neko.png', width=200)

with col2:
    #화면을 HTML로 만들어보기
    st.markdown(
        '''
        <h1 style='margin-bottom:0;'>AI 네코냥😸봇</h1>
...
#a. message 라는 이름의 변수가 st.session_state에 있는지 확인 후 첫 메세지 저장
if "messages" not in st.session_state:
    st.session_state.messages= [{'role':'assistant','content':'무엇이든 물어보세요.'}]

#b. 저장된 메세지들을 화면에 표시( 이전 메세지들이 ㅛ표시되는 역할)
...
if question:
    ...
    # 응답 - AI 응답요구 기능 함수 호출.... [응답할때까지 시간이 걸리기에...]
